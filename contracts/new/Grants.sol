...
interface Funds {
    function withdrawEthTo(address _to, uint256 _amount) external returns (bool);
}
...
contract Grants is Ownable {
    struct Request {
        address creator;
        string content;
        /// content URI saved onto IPFS in form of title , description and the media
        uint256 amountRequested;
        /// Amount in dollars
        uint256 amountApproved;
        /// AMount approved by the DAO
        bool approved;
        bool paid;
        uint256 yayVotes;
        uint256 nayVotes;
    ...
    function requestGrant(string memory contentURI, uint256 _amount) public onlyDAOMember {
        require(_amount > 0, "Amount is not right");
        GrantsRequests[_GrantsRequests] =
            Request(msg.sender, contentURI, _amount, 0, false, false, 0, 0, block.timestamp);
        _GrantsRequests += 1;
    }

    function vote(Vote _vote, uint256 _id) public onlyDAOMember {
        Request storage _request = GrantsRequests[_id];
        require(block.timestamp > _request.VotingStartTime, "Voting not Started ");
        require(block.timestamp < _request.VotingStartTime + votingDuration, "Voting has already ended");
    ...
    function endRequest(uint256 _id) public {
        Request storage _request = GrantsRequests[_id];
        require(block.timestamp > _request.VotingStartTime + votingDuration, "Voting has not yet ended");
    ...
    function _perform() public {
        for (uint256 id = 0; id < _GrantsRequests; id++) {
            ...
            require(
                block.timestamp > _request.VotingStartTime + votingDuration, "Voting hasn't ended yet for this member!"
            );
            endRequest(id);
        }
    }
    ...
    function transferFunds(uint256 _id) public onlyOwner {
        ...
        require(_grant.yayVotes > _grant.nayVotes, "The grant can not be processed");
        _grant.paid = true;
    ...
    function OpenForCollabrations(uint256 _requestId, address[] memory collaborators) public onlyDAOMember {
        require(_requestId <= _GrantsRequests, "Request does not exsist");
        collabrations[_collaborations] = Collabration(_requestId, msg.sender, collaborators, true);
    }

    function Collabrate(uint256 _id, address collaborator) public onlyDAOMember {
    ...
    function getCollaborations(uint256 _id) public returns (Collabration memory) {
