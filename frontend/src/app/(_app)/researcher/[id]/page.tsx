...
export default function ResearchPage({ params }: { params: { id: string } }) {
  ...
  const [member, setMember] = useState<PublishResearchValues>();
  const [researches, setResearches] = useState<ResearchValues[]>();
  ...
  const fetchMember = async (_id: bigint) => {
    try {
      setIsLoading(true);
      console.log(_id);
      ...
      const parsedRequest = {
        Id: Number(_id),
        ResearcherAddress: request.memberAddress,
        // Title: response.Name,
  ...
  const fetchMemberResearch = async (address: `0x${string}`) => {
    try {
      setIsLoading(true);
      console.log(address);
      const request = await publicClient?.readContract({
        ...
        address: DAOMember_Contract_Address,
        abi: DAOMember_ABI,
        functionName: "getMembersResearch",
        args: [address],
      });
      if (!request) {
        console.log("No request found");
      ...
      let promises: ResearchValues[] = [];

      request.forEach(async (research) => {
        const response = await fetchIPFS(research.researchPaperURI);
        console.log(response);
        const parsedRequest = {
          DateOfPublication: new Date(
            Number(research.dateOfPublication) * 1000
          ).toDateString(),
          CID: research.researchPaperURI,
          ...response,
        };
        console.log(parsedRequest);
        promises.push(parsedRequest);
      });
  ...
  useEffect(() => {
    if (params.id && !member && !isLoading) {
      fetchMember(BigInt(params.id));
    }
  }, [params.id]);
